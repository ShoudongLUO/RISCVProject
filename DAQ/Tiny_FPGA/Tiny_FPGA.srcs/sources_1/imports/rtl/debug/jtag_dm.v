...
module jtag_dm #(
    ...
    output wire jtag_pc_we_o;
    output wire [31:0] jtag_pc_wdata_o;
    ...
    reg jtag_pc_we;
    reg [31:0] jtag_pc_wdata;
    assign jtag_pc_we_o = jtag_pc_we;
    assign jtag_pc_wdata_o = jtag_pc_wdata;
    ...
    always @ (posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            read_data <= 32'h0;
            need_resp <= 1'b0;
            jtag_pc_we <= 1'b0;      // 确保复位时为低电平
            jtag_pc_wdata <= 32'h0;  // 复位为一个确定的值
            rx_data_r <= 32'h0;
            state <= STATE_IDLE;
        end else begin
            case (state)
