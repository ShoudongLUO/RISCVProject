...
`ifdef TEST_ADC
    reg serial_data_in;
    reg data_valid_in;
    reg [23:0] adc_test_data; // 24位测试数据（2通道×12位）
    reg [4:0] bit_counter;
    reg adc_data_active;
    ...
    initial begin
        ...
        forever begin
            ...
            for (bit_counter = 0; bit_counter < 24; bit_counter = bit_counter + 1) begin
                #40; // 每个bit 40ns (25MHz)
                serial_data_in = adc_test_data[23 - bit_counter];
                if (bit_counter == 23) begin
                    data_valid_in = 0;
                    adc_data_active = 0;
                end
            end
            adc_test_data = adc_test_data + 24'h010101; // 递增测试数据
        end
    end
`endif
...
`endif
    ...
    initial begin
        // 使用正确的路径指向你的指令文件
        $readmemh("D:/RISCV_Project/fpga/Testbench_bram/sdk/examples/my_adc/inst.data", 
                 tinyriscv_soc_top_0.u_rom.u_gen_ram.ram);
